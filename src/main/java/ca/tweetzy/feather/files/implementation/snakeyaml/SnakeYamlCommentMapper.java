package ca.tweetzy.feather.files.implementation.snakeyaml;

import ca.tweetzy.feather.files.comments.YamlCommentMapper;
import ca.tweetzy.feather.files.comments.YamlCommentParser;
import ca.tweetzy.feather.files.comments.format.YamlCommentFormatterConfiguration;
import ca.tweetzy.feather.files.configuration.comments.KeyTree;
import ca.tweetzy.feather.files.configuration.comments.format.CommentFormatterConfiguration;
...
public class SnakeYamlCommentMapper extends YamlCommentMapper {
    ...
    protected void setCommentPrefix(final YamlConfigurationOptions options) {
        final CommentFormatterConfiguration sideFormatterConfiguration = options.commentFormatter()
                .formatterConfiguration(ca.tweetzy.feather.files.configuration.comments.CommentType.SIDE);
    ...
    protected void trackBlockComment(final KeyTree.Node node, final List<CommentLine> comments) {
        if (node != null && comments != null && !comments.isEmpty()) {
            ...
            if (blockComment != null) {
                ...
                this.setRawComment(node, blockComment, ca.tweetzy.feather.files.configuration.comments.CommentType.SIDE);
            }
        }
    }
    ...
    protected void trackSideComment(final KeyTree.Node node, final List<CommentLine> comments) {
        if (node != null && comments != null && !comments.isEmpty()) {
            ...
            if (sideComment != null) {
                this.setRawComment(node, sideComment, ca.tweetzy.feather.files.configuration.comments.CommentType.SIDE);
            }
        }
    }
